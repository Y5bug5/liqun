...
(function($, window, document, undefined) {
	var Plugin = function(elem, options) {
		this.$wrapper = elem;
		this.$tab_list = this.$wrapper.find('#main_head').find('.click');
		this.$tabCont_wrap = this.$wrapper.find('#tabs');
		this.$tab_cont = this.$tabCont_wrap.find('.show_01');
		this.timer = null;
		this.playTimer = null
		this.iNow = 0;
		this.defaults = {
	...
	Plugin.prototype = {
		...
		tabInital: function() {
			...
			this.$tab_list.removeClass('click_first');
			this.$tab_list.eq(curNum).addClass('click_first');
		...
		changeTab: function(index) {
			this.$tab_list.removeClass('click_first');
			this.$tab_list.eq(index).addClass('click_first');
			switch(this.opts.changeMethod) {
				case 'default':
					this.$tab_cont.css({
						display: 'none'
